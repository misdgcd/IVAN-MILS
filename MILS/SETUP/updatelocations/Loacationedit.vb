...
Public Class Loacationedit
                ...
                Using excelCommand As New OleDbCommand(selectQuery, excelConnection)
                    Using excelReader As OleDbDataReader = excelCommand.ExecuteReader()
                        ...
                        While excelReader.Read()
                            Dim updateQuery As String = "UPDATE tblLocations " &
    "SET name = @Column2, remarks = @Column3, encby = @Column4, areaId = @Column5, lastModified = @Column6, apprvBy = @Column7 " &
    "WHERE id = @Column1"

                            Using updateCommand As New SqlCommand(updateQuery, connection)
                                ' Set the parameter values based on your Excel file columns
                                updateCommand.Parameters.AddWithValue("@Column1", excelReader("id"))
                                updateCommand.Parameters.AddWithValue("@Column2", excelReader("name"))
                                updateCommand.Parameters.AddWithValue("@Column3", excelReader("remarks"))
                                updateCommand.Parameters.AddWithValue("@Column4", excelReader("encby"))
                                updateCommand.Parameters.AddWithValue("@Column5", excelReader("areaId"))
                                updateCommand.Parameters.AddWithValue("@Column6", excelReader("lastModified"))
                                updateCommand.Parameters.AddWithValue("@Column7", excelReader("apprvBy"))
    ...
    Private Sub Loacationedit_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
